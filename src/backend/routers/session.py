...
async def get_insights(session_id: str):
    """
    Serve the precomputed insights artifact.

    insights.json is generated by scripts/generate_artifacts.py.
    This endpoint is a pure file-serve with no runtime AI generation.
    """
    import json
    ...
    if not insights_path.exists():
        raise HTTPException(
            status_code=404,
            detail=f"insights.json missing for {session_id}. Run scripts/generate_artifacts.py.",
        )
    ...
    try:
        ...
        logger.exception("Error reading insights.json for %s", session_id)
        raise HTTPException(status_code=500, detail=f"Failed to read insights.json: {e}")
...
async def get_track_replay(session_id: str):
    """
    Serve the precomputed track-replay artifact.

    track_replay.json is generated by scripts/generate_artifacts.py
    and committed into data/processed/<session_id>/.
    This endpoint is a pure file-serve with no runtime generation.
    """
    import json
    ...
    if not replay_path.exists():
        raise HTTPException(
            status_code=404,
            detail=f"track_replay.json missing for {session_id}. Run scripts/generate_artifacts.py.",
        )
    ...
    try:
        ...
        logger.exception("Error reading track_replay.json for %s", session_id)
        raise HTTPException(status_code=500, detail=f"Failed to read track_replay.json: {e}")
